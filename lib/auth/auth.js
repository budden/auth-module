...
export default class Auth {
  ...
  async loginWith (name, ...args) {
    await this.setStrategy(name);
    console.log('собираемся вызвать login из loginWith');
    const рез = await this.login(...args);
    console.log('login вернул ',рез);
    return рез;
  }
  ...
  async login () {
    ...
    // Здесь был callOnError, но мы его сломали. 
    await this.strategy.login(...arguments);
  }
  ...
  redirect (name, noRouter = false) {
    ...
    if (this.options.rewriteRedirects) {
      if (name === 'login' && isRelativeURL(from) && !isSameURL(to, from)) {
        this.setUniversal('redirect', from)
      }

      if (name === 'home') {
        const redirect = this.getUniversal('redirect')
        this.setUniversal('redirect', null)
